    ...
    export const uploadMusic = async (req, res) => {
        ...
        const AUDIO_FOLDER = path.join(__dirname, 'upload');

        try {
            const filename = req.params.filename;

            const filenamePath = req.params.filename.replace(".mp4","");
            ...
            const uploadStream = gfs.openUploadStream(filenamePath, {
                metadata: {
                    uploadedFrom: 'video folder',
                    uploadDate: new Date(),
                },
            });
